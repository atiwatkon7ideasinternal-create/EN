...
// วิชาที่เล่นอยู่: "vowel" (สระ) | "vocab" (คำศัพท์) — vocab-game.js เป็นคนสลับ
let currentSubject = "vowel";
...
function renderChoiceButtons(choices, correctVal, v) {
  const wrap = $("choices");
  wrap.className = "choices"; // รีเซ็ต (เผื่อมาจากโหมดคำศัพท์ที่เป็นคอลัมน์เดียว)
  wrap.innerHTML = "";
  choices.forEach((val) => {
...
$("btn-next").addEventListener("click", () => {
  if (currentSubject === "vocab") { vocabNext(); return; }
  state.index++;
